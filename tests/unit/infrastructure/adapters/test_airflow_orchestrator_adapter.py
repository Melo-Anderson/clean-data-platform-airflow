...
async def test_trigger_dag_calls_airflow_api() -> None:
    mock_resp = httpx.Response(200, json={"dag_run_id": "run_123"}, request=httpx.Request("POST", ""))
    
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch.object(AirflowOrchestratorAdapter, "_get_token", new_callable=AsyncMock) as mock_get_token:
        ...
        assert mock_post.call_args[0][0] == "http://airflow-webserver:8080/api/v2/dags/my-pipeline/dagRuns"
...
async def test_trigger_dag_retries_on_404() -> None:
    ...
    mock_200 = httpx.Response(200, json={"dag_run_id": "run_123"}, request=httpx.Request("POST", ""))
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        await adapter.trigger_dag(
            pipeline_id="p1", run_id="r1", dag_run_id="test__2026-01-01", pipeline_name="my-pipeline"
        )
