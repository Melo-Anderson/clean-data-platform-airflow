...
def test_validate_compute_execution_raises_when_job_result_is_none() -> None:
    with pytest.raises(RuntimeError, match="Compute job result is missing or None"):
        validate_compute_execution(job_result=None)
...
def test_quality_gate_passes_when_no_violations() -> None:
    with patch(
        "app.infrastructure.airflow_callbacks.shared_callbacks.QualityGateEvaluator"
    ) as mock_eval:
...
def test_success_notification_calls_adapter() -> None:
    with patch(
        "app.infrastructure.airflow_callbacks.shared_callbacks.NoopNotificationAdapter"
    ) as mock_adapter:
